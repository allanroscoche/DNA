...
é obtido. Esse método é chamado sequenciamento \emph{shotgun} \cite{Weber1997}.
...
sequenciadores tradicionais \cite{Harismendy2009} \cite{Ruffalo2012}.
...
\section{rsampling}
O \emph{rsampling} por padrão seleciona randomicamente fragmentos para diminuir a
cobertura para no máximo 300x. Um parâmetro necessário para a amostragem da cobertura é o tamanho esperado do código genético sendo sequenciado. Ele também estima a cobertura existente
necessário para a correção de erros.
...
Neste estágio, as leituras são convertidas do formato \emph{color space fasta(csfasta)} para o formato \emph{pseudobases}.
Nesse novo formato a primeira base e a primeira cor é retirada, após isso cada cor é convertido para uma letra correspondente.
As letras utilizadas nesse formato não representam as bases nucléicas das leituras, apenas as cores do \emph{colorspace}.
Um arquivo único chamado \emph{doubleEnconded\_input.de} é gerado. Esse arquivo é o único arquivo de entrada para o velvet, ou seja, as informações de qualidade não são utilizadas pelo montador.
A transformação para as bases é realizada apenas na fase final do pipeline.
...
Esse montador é o único que suporta a montagem com fragmentos em colorspace.
A única diferença do montador velvet \emph{colorspace} para o velvet \emph{basespace} está nas inversões de bases.
...
\subsection{velveth}
No velveth ocorre a fase de criação dos kmers, o arquivo com os framentos é
carregado na memória e os kmers são gerados.
O número de kmers gerado será a diferença do tamanho do fragmento pelo tamanho k, multiplicado pelo número de fragmentos.
Utilizando o exemplo da E.coli com cobertura de 600x, cada fragmento tem tamanho 50 e possui o total de 28629492 fragmentos. Utilizando $k$ com valor 31, teremos um total de 543 milhões de kmers para serem armazenados.
Uma tabela hash é utilizada para o armazenamento dos kmers para então o arquivo de saída Roadmap ser gerado.
A tabela hash tem um tamanho fixo de 16777216 entradas.
A função hash depende do valor máximo de kmer determinado na compilação do velvet.
Para $k$ menores que 32 a função hash é a divisão do kmer pelo tamanho da tabela.
Cada elemento da tabela é uma árvore Splay. Árvore splay é uma árvore binária equilibrada que tem como característica o acesso rápido a elementos que tenham sido acessados recentemente \cite{Splay}.
...
Esta estrutura é utilizada apenas para a construção, uma correção de erros simples e uma simplificação do grafo onde nós são concatenados.
...
quais parametros serão utilizados na montagem. Especialmente aqueles
...
erros é proposta baseada nas qualidades de cada base.
